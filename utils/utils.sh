...
readCredsFromFile() {
    if [ -f "$FILE" ]; then
        CREDS=$(cat $FILE)
        export DT_TENANT=$(echo $CREDS | jq -r '.dynatraceTenant')
        export DT_API_TOKEN=$(echo $CREDS | jq -r '.dynatraceApiToken')
        export DT_PAAS_TOKEN=$(echo $CREDS | jq -r '.dynatracePaaSToken')
    fi
}
printVariables(){
